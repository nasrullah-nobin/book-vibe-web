...
const ListedWishlistBook = ({ sortingType }) => {
  const { Wishlist } = useContext(BookContext);

  const filterWish = useMemo(() => {
  ...
  return filterWish.length === 0 ? (
    <div className="flex justify-center items-center bg-gray-200 h-[80vh]">
      <h2 className="text-2xl font-bold">No data found in wishlist</h2>
    </div>
  ) : (
    filterWish.map((book,ind) => (
      <div key={ind}
